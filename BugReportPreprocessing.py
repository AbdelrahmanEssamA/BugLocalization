...
class ReportPreprocessing:
    ...
    def stem(self):
        ...
        for report in self.bugReports.values():
            ...
            report.summary = dict(zip(['stemmed', 'unstemmed', 'lemma'], [[sbs.stem(token) for token in report.summary], report.summary, [lemmatizer.lemmatize(token) for token in report.summary]]))
            report.description = dict(zip(['stemmed', 'unstemmed', 'lemma'], [[sbs.stem(token) for token in report.description], report.description,  [lemmatizer.lemmatize(token) for token in report.description]]))
            report.pos_tagged_summary = dict(zip(['stemmed', 'unstemmed', 'lemma'], [[sbs.stem(token) for token in report.pos_tagged_summary], report.pos_tagged_summary,  [lemmatizer.lemmatize(token) for token in report.pos_tagged_summary]]))
            report.pos_tagged_description = dict(zip(['stemmed', 'unstemmed', 'lemma'], [[sbs.stem(token) for token in report.pos_tagged_description], report.pos_tagged_description,  [lemmatizer.lemmatize(token) for token in report.pos_tagged_description]]))
...
def main(data_set):
    ...
    with open(currentDataset.root + '/preprocessed_reports.pickle', 'wb') as file:
