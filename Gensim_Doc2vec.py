...
def main():
    ...
    with open('ZXing/preprocessed_src.pickle', 'rb') as file:
        src_files = pickle.load(file)
    with open('ZXing/preprocessed_reports.pickle', 'rb') as file:
        bug_reports = pickle.load(file)
    Sr = {'file_name': [src.fileName['unstemmed']
                        for src in src_files.values()],
          'class_name': [src.classNames['unstemmed']
                         for src in src_files.values()],
          'method_name': [src.methodNames['unstemmed']
                          for src in src_files.values()],
    ...
    df.to_csv(r'src.csv', index=None, header=True)
    rdf.to_csv(r'bugreps.csv', index=None, header=True)
    ...
    with open('ZXing/similarities4.json', 'w') as file:
